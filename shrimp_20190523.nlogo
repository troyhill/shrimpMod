...
globals [ bisc-outline     ; outline of BISC, vector shapefile
          ...
          salinity.data    ; time series data object - one value for entire bay. will be replaced with a folder of interpolated rasters at daily or 15-minute increments
...
turtles-own [ energy       ; shrimp health
              age          ; shrimp age (days)
...
to setup
  ...
  set bisc-outline gis:load-dataset "/home/thill/NetLogo 6.0.2/Projects/BISC_shrimp/BISC_GIS/BISC.shp"
  set bathymetry   gis:load-dataset "/home/thill/NetLogo 6.0.2/Projects/BISC_shrimp/BISC_GIS/bisc_bath.asc"
  ...
  ; set submerged-area patches with [ gis:property-value bisc-outline "ACRES" ]
  ;ask patches [ set submerged bathymetry  < 0]


  let number-of-turtles 200

  create-turtles number-of-turtles [
    setxy random-pxcor random-pycor
    ;; select random coordinates, then check that they're inside the bay
    ; let gx gis:raster-value bathymetry random-pxcor random-pycor
    ; if ((gx <= 0) and (gx >= 0)) [die] ;; if this is true, the values gx and gy are valid

      ; set xcor gis:raster-sample bathymetry pxcor
      ; set ycor gis:raster-sample bathymetry pycor
  ...
  ;let gx  gis:raster-value bathymetry random-pxcor random-pycor
  ;ask patches with [ gx >= 0  and  gx >= 0]
  ;[ set pcolor red ]
...
to look-ahead
;------------------------------------------------------------------------------
ifelse [ submerged ] of patch-ahead 1 = "NaN"
      [ lt random-float 360 ]   ;; See a black patch ahead : turn left randomly
...
;-------------------------------------------------------------------------
 ...
 ; set energy          energy_init
 ; set age             age_init
 ; set life-stage      life-stage_init
...
to shrimp-movement  ;; turtle procedure -- chill when off black patches
 ; ask patch-here
   ...
   if submerged = "NaN"
    [ wiggle ]
  ; [ hatch-dead-shrimp 1] ; if ((gx <= 0) and (gx >= 0))
end
